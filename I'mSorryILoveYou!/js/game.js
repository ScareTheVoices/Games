...
(function (YG) {
  ...
  const WALK_STEP_SECONDS = 0.14; // time between walk-cycle frame swaps
  const WALK_FRAME_CYCLE = [0, 1, 2, 1]; // middle frame (1) is the idle pose

  // The uploaded character sheet — a standard RPG Maker MV/MZ multi-actor
  // sheet (12x8 frames, up to 8 characters). This one only has slot 0 filled.
  const PLAYER_SPRITE = { path: 'assets/characters/player.png', mode: 'multi', index: 0 };
  ...
  const state = {
    ...
    keys: {},
    pos: { x: 0, y: 0 },
    npcSprites: {}, // npcId -> { el, x, y, character }
    lastFrameTime: 0,
    dialogueOpen: false,
    menuOpen: false,
    nearestNpcId: null,
  ...
  async function init() {
    ...
    try {
      state.playerSpriteDims = await YG.Sprites.preload(PLAYER_SPRITE.path);
      state.playerHasSprite = true;
    } catch (err) {
      console.warn('Player sprite sheet not available, falling back to a plain marker.', err);
      state.playerHasSprite = false;
    }
  ...
  function updateFacingAndAnimation(dx, dy, moving, dt) {
    if (moving) {
      ...
      state.walkAnimAccum += dt;
      if (state.walkAnimAccum >= WALK_STEP_SECONDS) {
        state.walkAnimAccum = 0;
        state.walkCyclePos = (state.walkCyclePos + 1) % WALK_FRAME_CYCLE.length;
        state.walkFrame = WALK_FRAME_CYCLE[state.walkCyclePos];
      }
    } else {
      state.walkFrame = 1;
      state.walkCyclePos = 0;
      state.walkAnimAccum = 0;
    }
    updatePlayerSpriteVisual();
  }
  ...
  function updatePlayerSpriteVisual() {
    if (!state.playerHasSprite) return;
    YG.Sprites.applyToElement(
      els.playerEl,
      PLAYER_SPRITE,
      state.playerSpriteDims,
      state.facing,
      state.walkFrame,
      SPRITE_SIZE
    );
  ...
  function updateMovement(dt) {
    ...
    updateFacingAndAnimation(dx, dy, moving, dt);
  ...
  function renderNpcsForPhase(phase) {
    ...
    Object.entries(byRoom).forEach(([roomId, npcs]) => {
      ...
      npcs.forEach((character, idx) => {
        const spacing = SPRITE_SIZE + 6;
        const cols = Math.max(1, Math.floor((room.w - 20) / spacing));
        const col = idx % cols;
        const row = Math.floor(idx / cols);
        const x = room.x + 16 + col * spacing;
        const y = room.y + 34 + row * spacing;
  ...
  function spawnNpcSprite(character, x, y) {
    ...
    if (character.sprite) {
      // Optional per-character art (same RPG Maker sheet format as the
      // player). Falls back silently to the initials circle if it fails.
      YG.Sprites.preload(character.sprite.path)
        .then((dims) => {
          YG.Sprites.applyToElement(el, character.sprite, dims, 'down', 1, SPRITE_SIZE);
        })
