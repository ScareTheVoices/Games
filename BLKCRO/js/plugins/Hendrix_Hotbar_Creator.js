 ...
 * 
 * Video Tutorial: https://www.youtube.com/watch?v=XUcPENaVu4M
 * ----------------------------------------------------------------------------
 * 
...
(() => {
    ...
    Sprite_SkillSlot.prototype.updatePlayerOverlapFade = function () {
        if (!this._playerFadeOpacity) this._playerFadeOpacity = 1.0;

        const isBehind = this.isPlayerBehindSlot();

        if (isBehind) {
            this._playerFadeOpacity = Math.max(0.3, this._playerFadeOpacity - 0.05);
        } else {
            this._playerFadeOpacity = Math.min(1.0, this._playerFadeOpacity + 0.05);
        }

        this.alpha = this._playerFadeOpacity;
    };
    ...
    Sprite_SkillSlot.prototype.isPlayerBehindSlot = function () {
        if (!hideWhenOnPlayer) return false;
        if (!SceneManager._scene || !SceneManager._scene._spriteset) return false;
        ...
        const slotLeft = this.x - halfW;
        const slotRight = this.x + halfW;
        const slotTop = this.y - halfH;
        const slotBottom = this.y + halfH;

        return (
            playerRight > slotLeft &&
            playerLeft < slotRight &&
            playerBottom > slotTop &&
            playerTop < slotBottom
        );
    };

    Spriteset_SkillUI.prototype.updatePlayerOverlapFade = function () {
        this.alpha = this._fadeOpacity / 255;
    };
    ...
    function createHotbarEditorButton() {
        if (!showHotbarButton) return;
        if (!Utils.isOptionValid('test')) return;
        if (!Utils.isNwjs()) return;
        if (document.getElementById('hotbarEditorButton')) return;
        ...
        const findAvailablePosition = () => {
            ...
            const existingButtons = document.querySelectorAll('[id$="EditorButton"]');
            const occupiedPositions = Array.from(existingButtons).map(btn => {
                const bottom = parseInt(btn.style.bottom) || baseBottom;
                return bottom;
            });

            let position = baseBottom;
            while (occupiedPositions.includes(position)) {
                position += buttonHeight + gap;
            }
        ...
        button.id = 'hotbarEditorButton';
        button.innerHTML = '⏹️';

        const bottomPosition = findAvailablePosition();
